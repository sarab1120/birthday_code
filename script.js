...
function typeMessage() {
    const messages = [
        `Dear ${userName},`,
    "You light up the world in ways you’ll never know 🌟",
    "Your laughter stays with me long after it ends 😌",
    "Your heart is rare, your spirit unstoppable 💛",
    "Every moment with you feels like magic ✨",
    "I’m grateful for you today, tomorrow, always 🤍",
...
function createFireworks() {
    for (let i = 0; i < FIREWORK_CONFIG.initialCount; i++) {
        setTimeout(createFirework, i * 600);
    }

    setInterval(() => {
        if (!document.hidden && Math.random() < FIREWORK_CONFIG.probability) {
            createFirework();
        }
    }, FIREWORK_CONFIG.interval);
}

function createFirework() {
    const x = random(50, window.innerWidth - 50);
    const y = random(50, window.innerHeight * 0.75);
    ...
    setTimeout(() => explode(x, y, color), 700);
    setTimeout(() => firework.remove(), FIREWORK_CONFIG.duration);
}

function explode(x, y, color) {
    const fragment = document.createDocumentFragment();

    for (let i = 0; i < FIREWORK_CONFIG.particles; i++) {
        const angle = random(0, Math.PI * 2);
        const distance = random(60, FIREWORK_CONFIG.maxDistance);
        ...
        setTimeout(() => particle.remove(), FIREWORK_CONFIG.duration);
    }
